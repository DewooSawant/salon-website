...
import { FiPhone, FiArrowLeft, FiArrowRight, FiShield, FiCheck, FiScissors } from 'react-icons/fi'
...
export default function SalonOwnerLogin() {
  ...
  const handleSendOTP = async () => {
    ...
    setLoading(true)
    try {
      const res = await axios.post(`${API_URL}/otp/send`, { phone: cleaned, role: 'salon_owner' })
  ...
  const handleVerifyOTP = async (otpValue) => {
    ...
    setLoading(true)
    try {
      const res = await axios.post(`${API_URL}/otp/verify/salon-owner`, {
        phone: phone.replace(/\D/g, ''), otp: otpStr,
      })
            ...
            {step === 1 && (
              <motion.div key="phone" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                <div className="text-center mb-8">
                  ...
                  <p className="text-gray-500 text-sm">Enter the phone number linked to your salon</p>
                </div>
                ...
                <div className="mt-6 pt-6 border-t border-gray-100 text-center space-y-2">
                  ...
                  <Link to="/login" className="block text-sm text-gray-400 hover:text-gray-600">Customer login</Link>
                </div>
              </motion.div>
            )}
            ...
            {step === 2 && (
              <motion.div key="otp" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                ...
                <button onClick={() => handleVerifyOTP()} disabled={loading || otp.join('').length !== 6}
                  ...
                  {loading ? <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" />
                    : <>Verify & Login <FiCheck className="w-4 h-4" /></>}
                </button>
